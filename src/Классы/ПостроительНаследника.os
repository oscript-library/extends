...
Функция Построить() Экспорт
	...
	Если КонструкторНаследника <> Неопределено ИЛИ КонструкторРодителя <> Неопределено Тогда
		...
		Если КонструкторНаследника <> Неопределено Тогда
			ПеренестиАннотации(Метод, КонструкторНаследника.Аннотации, "Наследует");
		КонецЕсли;

		Если КонструкторРодителя <> Неопределено Тогда
			ПеренестиАннотации(Метод, КонструкторРодителя.Аннотации, "Наследует");
		КонецЕсли;

		Декоратор.Метод(Метод);
	КонецЕсли;

	Декоратор.ЗарегистрироватьВСистемеТипов("" + ТипЗнч(_ОбъектНаследник) + "_наследует_" + ИмяТипаРодителя);
